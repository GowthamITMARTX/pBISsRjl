...
ajax_model = {
    input_tag: null,
    init: function () {
        $('.model-link').attr("autocomplete", "off")
            ...
            .on('blur', function (e) {
                ...
                if (self.val().length > 0) {
                    $('#myModal').modal({
                        remote: self.data('url') + "?key=" + encodeURIComponent(self.val()) + "&type=" + self.data('type'),
                        show: 'false'
                    });
                }
            }).on('keyup', function (e) {
             //   e.preventDefault();
                if (!$($(this).data('hide')).hasClass('hidden'))
        ...
        $('body').on('click', '.tr-list', function () {
            obj = $(this).data('object');

            if($(this).data('for') == 'refresh'){
               // console.log($(this).data('refresh'));
                location.href = URL.current + "?code=" + obj.title ;
            }

            input_tag.val(obj.title);
            ...
            if (input_tag.data('id') == 'cls_id')  ajax_cls_subject.init(obj.id);
            if ($(input_tag.data('hide')).hasClass('hidden'))
                $(input_tag.data('hide')).removeClass('hidden');
            $('#myModal').modal('hide');
